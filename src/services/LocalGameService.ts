...
export class LocalGameService {
  private getStoredGames(): StoredGame[] {
    const stored = localStorage.getItem('alchemist_local_games');
    if (!stored) return [];
    try {
  ...
  private saveStoredGames(games: StoredGame[]) {
    localStorage.setItem('alchemist_local_games', JSON.stringify(games));
  }
  ...
  public async getGame(id: string): Promise<{ success: boolean; game?: any; connectedPlayers?: string[]; error?: string }> {
    ...
    if (!found) {
      return { success: false, error: 'Crucible session not found.' };
    }
    let state: GameState;
    try {
    ...
    return {
      ...
      connectedPlayers: ['apprentice@local']
    };
  }
  ...
  public async createGame(name: string, maxPlayers: number, maxTicks: number): Promise<{ success: boolean; gameId?: string; inviteCode?: string; name?: string; error?: string }> {
    const hostName = localStorage.getItem('alchemist_display_name') || 'Apprentice';
    let state: GameState;
    try {
      ...
      return { success: false, error: 'Failed to initialize crucible.' };
    }

    // Ensure players are marked local
    state.players = state.players.map(p => {
      if (p.id === 'player_1') {
        return { ...p, isLocal: true, assignedEmail: 'apprentice@local' };
      }
  ...
  public async performGameAction(gameId: string, action: any, playerId: string): Promise<{ success: boolean; gameState?: GameState; error?: string }> {
    ...
    if (nextState.status === 'completed' && state.status !== 'completed') {
      const hostName = localStorage.getItem('alchemist_display_name') || 'Apprentice';
      const key = `alchemist_offline_stats_${hostName}`;
