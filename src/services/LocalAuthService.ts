export class LocalAuthService {
  ...
  public async recordGameStats(won: boolean): Promise<void> {
    const displayName = localStorage.getItem('alchemist_display_name') || 'Apprentice';
    const key = `alchemist_offline_stats_${displayName}`;
  ...
  private getLocalUser(): any {
    ...
    let stats = { gamesPlayed: 0, gamesWon: 0 };
    try {
      const storedStats = localStorage.getItem(`alchemist_offline_stats_${displayName}`);
      if (storedStats) {
