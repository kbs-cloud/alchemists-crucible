...
APP_NAME="alchemists-crucible"
DEFAULT_OFFSET=4  # Offset in the port ranges
...
echo "=== Starting Alchemist's Crucible Deployment ($DEPLOY_ENV) ==="
...
if [ "$DEPLOY_ENV" == "production" ]; then
    ...
    SERVICE_DESC="Alchemist's Crucible Production Service"
else
    ...
    SERVICE_DESC="Alchemist's Crucible Dev/Testing Service"
fi
...
Restart=always
Environment=NODE_ENV=production BACKEND_PORT=$BACKEND_PORT FRONTEND_PORT=$FRONTEND_PORT DATABASE_PATH=$DEPLOY_DIR/alchemists_crucible.db AUTH_SERVER_URL=http://localhost:20001 HUB_API_URL=http://localhost:20000 HUB_APP_TOKEN=alchemist_token_dev_777
