...
const PORT = process.env.BACKEND_PORT || 29004;

// Initialize SQLite database
const dbPath = process.env.DATABASE_PATH || path.join(__dirname, 'alchemists_crucible.db');
...
app.get('/api/auth/callback', async (req, res) => {
  ...
  try {
    ...
    const tokenRes = await fetch(`${authServerUrl}/api/auth/token`, {
      ...
      body: JSON.stringify({ code, client_id: 'alchemist' })
    });
...
app.post('/api/auth/backchannel-logout', async (req, res) => {
  ...
  try {
    ...
    db.run('DELETE FROM sessions WHERE email = ?', [email], (err) => {
      ...
      console.log(`[Back-Channel Logout] Cleared local alchemist sessions for ${email}`);
...
const HUB_APP_TOKEN = process.env.HUB_APP_TOKEN || 'alchemist_token_dev_777';
    ...
    db.run(
      ...
      (updErr) => {
        ...
        db.get('SELECT games_played, games_won FROM users WHERE email = ?', [user.email], (selErr, row) => {
          ...
          if (won) {
            unlockHubAchievement(user.email, 'alchemist_philosophers_stone');
          }
...
app.post('/api/games', validateCSRF, (req, res) => {
  getSessionUser(req, (err, user) => {
    ...
    const maxTicks = setupOptions?.maxTicks || 25;
...
app.post('/api/games/:gameId/assign-slot', validateCSRF, (req, res) => {
  ...
  db.get('SELECT game_state FROM games WHERE id = ?', [gameId], (err, game) => {
    ...
    if (isAi) {
      player.isAi = true;
      player.assignedEmail = `ai_${playerId}@alchemist.ai`;
      player.name = name || `AI Apprentice`;
  ...
  getSessionUser(req, (err, user) => {
    ...
    db.get('SELECT owner_email, game_state FROM games WHERE id = ?', [gameId], (dbErr, game) => {
      ...
      if (action.type === 'start') {
        if (!isOwner) {
          return res.status(403).json({ error: 'Only the game host can ignite the crucible.' });
        }
      } else {
        if (!isAuthorized) {
          return res.status(403).json({ error: 'Unauthorized action for this apprentice slot.' });
        }
      }

      // Execute alchemical action in state machine
      ...
      db.run('UPDATE games SET game_state = ?, updated_at = ? WHERE id = ?', [nextStateStr, new Date().toISOString(), gameId], (updErr) => {
        ...
        if (nextState.status === 'completed' && state.status !== 'completed') {
          // Find the player who synthesized the ultimate element (gold/philosopher's stone/ai)
          const winningPlayer = nextState.players.find(p => p.inventory.includes('philosophers_stone') || p.inventory.includes('ai'));
          if (winningPlayer && winningPlayer.assignedEmail) {
            unlockHubAchievement(winningPlayer.assignedEmail, 'alchemist_ultimate_transmutation');
          }
        }
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Alchemist's Crucible backend listening on port ${PORT}`);
});
...
if (process.env.FRONTEND_PORT && String(process.env.FRONTEND_PORT) !== String(PORT)) {
  ...
  frontendApp.all('/api/*splat', (req, res) => {
    ...
    connector.on('error', (err) => {
      console.error('Alchemist\'s Crucible frontend proxy error:', err);
  ...
  frontendApp.listen(process.env.FRONTEND_PORT, () => {
    console.log(`Alchemist's Crucible static frontend server running on port ${process.env.FRONTEND_PORT}`);
  });
}
